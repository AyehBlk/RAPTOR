...
### 🎯 Ensemble Analysis - Combine Multiple DE Methods

**Stop relying on a single method!** RAPTOR v2.2.0 introduces **Module 9: Ensemble Analysis** with 5 statistical methods to combine results from DESeq2, edgeR, and limma for robust, publication-quality findings.

```python
from raptor import ensemble_brown, ensemble_fisher, ensemble_rra

# Combine results using Brown's method (accounts for correlation)
...
**5 Ensemble Methods:**
- 🎲 **Fisher's Method** - Classic p-value combination
- 📊 **Brown's Method** - Correlation-aware (recommended)
- 🏆 **Robust Rank Aggregation (RRA)** - Outlier-robust
- 🗳️ **Voting Consensus** - Conservative agreement
- ⚖️ **Weighted Ensemble** - Method-specific weights

### ⚙️ Advanced Parameter Optimization - 4 Methods

**Module 8: Parameter Optimization** provides data-driven threshold optimization using 4 validated approaches:

```python
from raptor import optimize_with_fdr_control

# Optimize thresholds to achieve target FDR
result = optimize_with_fdr_control(
    de_result=de_result,
    fdr_target=0.05
)

print(f"Optimal thresholds: FDR={result.optimal_threshold['padj']:.3f}, |logFC|={result.optimal_threshold['lfc']:.2f}")
```

**4 Optimization Methods:**
1. **Ground Truth** - Maximize F1 with known positives (simulated/spike-in data)
2. **FDR Control** - Achieve target FDR using Storey's π₀ estimation
3. **Stability** - Cross-validation stability analysis
4. **Reproducibility** - Agreement across independent cohorts

### 📊 32-Feature Data Profiling

**Module 3: Data Profiling** extracts 32 statistical features powering ML-based recommendations:

```python
from raptor import profile_data_quick

profile = profile_data_quick('counts.csv', 'metadata.csv', group_column='condition')
print(f"BCV: {profile.bcv:.3f}")  # The most important metric!
print(f"Category: {profile.bcv_category}")
```

**Key Metrics:**
- **BCV (Biological Coefficient of Variation)** - Guides pipeline selection
- Sample size & balance
- Dispersion patterns
- Sparsity & zero inflation  
- 28 additional features for ML

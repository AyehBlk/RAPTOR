...
def sample_de_result(sample_de_data):
    ...
    if DE_IMPORT_AVAILABLE:
        ...
        # Create a simple mock if DEResult not available
        class MockDEResult:
            def __init__(self, data, method):
                self.data = data
                self.method = method
        result = MockDEResult(data=sample_de_data, method='DESeq2')
...
def multiple_de_results(sample_de_data):
    ...
    for i, method in enumerate(['DESeq2', 'edgeR', 'limma']):
        ...
        if DE_IMPORT_AVAILABLE:
            results[method] = DEResult(data=data, method=method, metadata={})
        else:
            class MockDEResult:
                def __init__(self, data, method):
                    self.data = data
...
class TestFisherMethod:
    ...
    def test_fisher_output_files(self, multiple_de_results, tmp_output_dir):
        ...
        assert (tmp_output_dir / 'method_statistics.json').exists()
        assert (tmp_output_dir / 'parameters.json').exists()
...
class TestBrownMethod:
    ...
    def test_brown_vs_fisher(self, multiple_de_results, tmp_output_dir):
        ...
        # (Brown accounts for correlation)
        assert fisher_result.n_consensus_genes != brown_result.n_consensus_genes or \
               set(fisher_result.consensus_genes['gene_id']) != set(brown_result.consensus_genes['gene_id'])
...
class TestVoting:
    ...
    def test_voting_majority(self, multiple_de_results, tmp_output_dir):
        """Test majority voting (≥2 methods)."""
        result = ensemble_voting(
            de_results=multiple_de_results,
            min_methods=2,
            filters={'padj': 0.05},
        ...
        assert result.n_consensus_genes > 0
        assert 'n_methods' in result.consensus_genes.columns
    ...
    def test_voting_threshold_effect(self, multiple_de_results, tmp_output_dir):
        ...
        result_2 = ensemble_voting(
            ...
            min_methods=2,
            filters={'padj': 0.05},
            output_dir=tmp_output_dir / "min2"
        )
        ...
        result_3 = ensemble_voting(
            ...
            min_methods=3,
            filters={'padj': 0.05},
            output_dir=tmp_output_dir / "min3"
        )
        
        # Higher threshold should give fewer or equal genes
        assert result_3.n_consensus_genes <= result_2.n_consensus_genes
    ...
    def test_voting_with_lfc_filter(self, multiple_de_results, tmp_output_dir):
        ...
        # All genes should have |LFC| >= 1.0
        if len(result.consensus_genes) > 0:
            lfcs = result.consensus_genes['meta_lfc'].abs()
            assert lfcs.min() >= 1.0
...
class TestWeightedEnsemble:
    ...
    def test_weighted_equal_weights(self, multiple_de_results, tmp_output_dir):
        ...
        assert result.n_consensus_genes >= 0
        assert 'weighted_score' in result.consensus_genes.columns
    ...
    def test_weighted_min_score_effect(self, multiple_de_results, tmp_output_dir):
        ...
        result_high = ensemble_weighted(
            ...
            weights=weights,
            min_score=2.0,
            output_dir=tmp_output_dir / "high"
        )
        
        # Higher min_score should give fewer or equal genes
        assert result_high.n_consensus_genes <= result_low.n_consensus_genes
...
class TestEdgeCases:
    ...
    def test_empty_results(self, tmp_output_dir):
        ...
        if DE_IMPORT_AVAILABLE:
            de_results = {
                'DESeq2': DEResult(data=empty_data, method='DESeq2', metadata={})
            }
        else:
            class MockDEResult:
                def __init__(self, data, method):
    ...
    def test_single_method(self, sample_de_result, tmp_output_dir):
        ...
        # Should work but may have limited usefulness
        assert result.n_consensus_genes >= 0
    ...
    def test_invalid_weights(self, multiple_de_results, tmp_output_dir):
        ...
        weights = {
            'DESeq2': 0.9,
            'edgeR': -0.5,  # Invalid negative weight
            'limma': 0.8
        }
...
class TestIntegration:
    ...
    def test_overlap_analysis(self, multiple_de_results, tmp_output_dir):
        ...
        voting_result = ensemble_voting(
            ...
            min_methods=3,
            filters={'padj': 0.05},
            output_dir=tmp_output_dir / "voting"
        )
        
        # Voting (unanimous) should be subset of Fisher's
        if voting_result.n_consensus_genes > 0:
            fisher_genes = set(fisher_result.consensus_genes['gene_id'])
            voting_genes = set(voting_result.consensus_genes['gene_id'])
...
class TestPerformance:
    ...
    def test_large_dataset(self, tmp_output_dir):
        ...
        for method in ['DESeq2', 'edgeR', 'limma']:
            ...
            if DE_IMPORT_AVAILABLE:
                de_results[method] = DEResult(data=data, method=method, metadata={})
            else:
                class MockDEResult:
                    def __init__(self, data, method):
                        self.data = data

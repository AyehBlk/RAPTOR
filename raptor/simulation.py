...
class SimulationResult:
    ...
    def __post_init__(self):
        ...
        validate_positive_integer(self.n_genes, 'n_genes')
        validate_positive_integer(self.n_samples_per_group, 'n_samples_per_group')
        validate_probability(self.de_fraction, 'de_fraction')
        if self.dispersion_mean <= 0:
            raise ValidationError(f'dispersion_mean must be > 0, got {self.dispersion_mean}')
        if self.dispersion_sd < 0:
            raise ValidationError(f'dispersion_sd must be >= 0, got {self.dispersion_sd}')
        if self.library_size_mean <= 0:
            raise ValidationError(f'library_size_mean must be > 0, got {self.library_size_mean}')
        if self.library_size_cv < 0:
            raise ValidationError(f'library_size_cv must be >= 0, got {self.library_size_cv}')
        if not 0 <= self.dropout_rate <= 1:
            raise ValidationError(f'dropout_rate must be 0-1, got {self.dropout_rate}')
        if not 0 <= self.outlier_fraction <= 1:
            raise ValidationError(f'outlier_fraction must be 0-1, got {self.outlier_fraction}')

...
class QuickSalmonPipeline:
    ...
    def __init__(
        ...
        # =====================================================================
        # VALIDATION (8-10 checks - lightweight for QC)
        # =====================================================================
        
        logger.info("🔍 Validating inputs...")
        
        # 1. Sample sheet exists and is valid
        validate_file_path(
        ...
        validate_directory_path(self.index, 'index')
        ...
        if not (self.index / "versionInfo.json").exists():
            raise ValidationError(
                ...
                f"\n"
                f"Rebuild index with:\n"
                f"  salmon index -t transcripts.fa -i {self.index}\n"
                f"\n"
                f"Download transcriptome:\n"
                f"  - Ensembl: https://www.ensembl.org/\n"
                f"  - GENCODE: https://www.gencodegenes.org/"
            )
        
        # 4. Threads must be positive
        validate_positive_integer(self.threads, 'threads', min_value=1, max_value=128)
        
        # 5. Library type must be valid
        validate_choice(
            self.lib_type,
            'lib_type',
            valid_choices=VALID_LIBRARY_TYPES,
            hint=f"Valid library types: {', '.join(VALID_LIBRARY_TYPES)}. "
                 "Use 'A' for auto-detect (recommended)."
        )
        ...
        if self.gene_map:
            validate_file_path(
                ...
                hint="Gene map should be TSV/CSV with columns: transcript_id, gene_id"
            )
        
        # 7. Fragment length validation (for single-end)
        validate_positive_integer(
            self.fragment_length_mean,
            'fragment_length_mean',
            min_value=50,
            max_value=1000
        )
        validate_positive_integer(
            self.fragment_length_sd,
            'fragment_length_sd',
            min_value=10,
            max_value=500
        )
        ...
        try:
            ...
            raise ValidationError(
                f"Failed to load sample sheet: {sample_sheet}\n"
                f"Error: {str(e)}\n"
                f"\n"
                f"Sample sheet must be a CSV file with columns:\n"
                f"  - sample_id: Unique sample identifier\n"
                f"  - condition: Experimental condition (e.g., Control, Treatment)\n"
                f"  - batch: Batch number (optional)\n"
                f"  - fastq_r1: Path to R1 FASTQ file\n"
                f"  - fastq_r2: Path to R2 FASTQ file (empty for single-end)\n"
                f"\n"
                f"Example format:\n"
                f"  sample_id,condition,batch,fastq_r1,fastq_r2\n"
                f"  Sample1,Control,1,data/S1_R1.fq.gz,data/S1_R2.fq.gz"
            )
        ...
        if not is_valid:
            ...
            raise ValidationError(
                f"Sample sheet validation failed: {sample_sheet}\n"
                f"\n"
                f"Errors found:\n"
                f"{error_msg}\n"
                f"\n"
                f"Common issues:\n"
                f"  - FASTQ files not found at specified paths\n"
                f"  - Incorrect file paths (check absolute vs relative)\n"
                f"  - Missing required columns\n"
                f"  - Duplicate sample IDs\n"
                f"\n"
                f"Troubleshooting:\n"
                f"  1. Check all FASTQ file paths exist\n"
                f"  2. Use absolute paths or paths relative to where you run RAPTOR\n"
                f"  3. Verify CSV format is correct (no extra commas)"
            )
        ...
        if not self._check_salmon_installed():
            raise ValidationError(
                ...
                f"\n"
                f"Salmon is required for quantification but is not installed.\n"
                f"\n"
                f"Install with conda/mamba:\n"
                f"  conda install -c bioconda salmon\n"
                f"  # OR\n"
                f"  mamba install -c bioconda salmon\n"
                f"\n"
                f"Verify installation:\n"
                f"  salmon --version\n"
                f"\n"
                f"Required version: >= 1.5.0\n"
                f"Recommended version: >= 1.9.0 (latest features)"
            )
    ...
    def run(self) -> bool:
        ...
        if not any(r.success for r in self.results):
            raise PipelineError(
                ...
                step="salmon_quant",
                hint="Check Salmon log files in output directory"
            )

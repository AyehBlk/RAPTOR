...
class QuickKallistoPipeline:
    ...
    def __init__(
        ...
        """Initialize Quick Kallisto Pipeline.
        ...
        fragment_length and fragment_sd can be passed directly as kwargs
        OR inside the config dict. Direct kwargs take priority.
        """
        ...
        self.gene_map = str(gene_map) if gene_map else None
        self.config = config or {}
        
        # Extract config values — direct kwargs take priority over config dict
        self.fragment_length = fragment_length if fragment_length is not None else self.config.get('fragment_length', 200)
        self.fragment_sd = fragment_sd if fragment_sd is not None else self.config.get('fragment_sd', 20)
    ...
    def validate_gene_map(self) -> None:
        ...
        gene_map_path = Path(self.gene_map)
    ...
    def validate_single_end_requirements(self) -> None:
        ...
        if self._sample_sheet_obj and not self._sample_sheet_obj.is_paired:
    ...
    @handle_errors(exit_on_error=False)  # v2.2.0 error handling
    def run(self, parallel: bool = False) -> bool:
        ...
        # Validate Kallisto installed
        self.validate_kallisto_installed()
        
        # Validate single-end requirements
        ...
        samples = list(self._sample_sheet_obj.samples)

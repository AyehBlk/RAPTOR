...
class SampleInfo:
    ...
    def fastq_r1(self) -> Path:
        """Alias for fastq_1 (R1 FASTQ file)."""
        return self.fastq_1
    
    @property
    def fastq_r2(self) -> Optional[Path]:
        """Alias for fastq_2 (R2 FASTQ file)."""
        return self.fastq_2
...
class PipelineResult:
    ...
    def summary(self) -> str:
        ...
        n_samples = self.n_samples_processed if self.n_samples_processed else len(self.sample_results)
        lines.append(f"📋 Samples processed: {n_samples}")
        
        if self.elapsed_time:
            lines.append(f"⏱️  Duration: {self.elapsed_time:.1f} sec")
...
def auto_detect_samples(
    ...
    1. Classify files using explicit R1/R2 markers (_R1, .R1, _R1_001)
    2. For unclassified files, try _1/_2 pairing as fallback
    3. Remaining unclassified files treated as single-end
    
    Parameters
    ----------
    fastq_dir : str or Path
        Directory containing FASTQ files
    pattern : str or list, optional
        Glob pattern(s) for FASTQ files
    
    Returns
    -------
    Tuple[List[SampleInfo], str]
        (samples, read_type) where read_type is 'paired', 'single', or 'unknown'
    """
    import re
    ...
    all_files = []
    for pat in patterns:
        all_files.extend(fastq_dir.glob(pat))
    ...
    # =========================================================================
    # Pass 1: Classify using EXPLICIT R1/R2 markers
    # =========================================================================
    r1_markers = [r'_R1_001\.', r'_R1\.', r'\.R1\.', r'_R1_']
    r2_markers = [r'_R2_001\.', r'_R2\.', r'\.R2\.', r'_R2_']
    ...
    for f in all_files:
        name = f.name
        is_r1 = any(re.search(p, name, re.IGNORECASE) for p in r1_markers)
        is_r2 = any(re.search(p, name, re.IGNORECASE) for p in r2_markers)
        
        if is_r1 and not is_r2:
            r1_files.append(f)
        elif is_r2 and not is_r1:
            r2_files_set.add(str(f))
        else:
            unclassified.append(f)
    
    # =========================================================================
    # Pass 2: Try _1/_2 pairing on unclassified files
    # Only accept if ALL unclassified files pair cleanly (no leftovers)
    # This avoids false-pairing Sample_1/Sample_2 when Sample_3 also exists
    # =========================================================================
    if unclassified:
        num1_files = {}  # base_name -> file
        num2_files = {}  # base_name -> file
        no_number = []
        
        for f in unclassified:
            name = f.name
            if re.search(r'_1\.', name):
                base = re.sub(r'_1\.', '_*.', name)
                num1_files[base] = f
            elif re.search(r'_2\.', name):
                base = re.sub(r'_2\.', '_*.', name)
                num2_files[base] = f
            else:
                no_number.append(f)
        
        # Only pair if: every _1 has a _2, every _2 has a _1, and no leftovers
        all_pair = (
            len(num1_files) > 0
            and len(num1_files) == len(num2_files)
            and set(num1_files.keys()) == set(num2_files.keys())
            and len(no_number) == 0
        )
        
        if all_pair:
            for base, f1 in num1_files.items():
                r1_files.append(f1)
                r2_files_set.add(str(num2_files[base]))
            unclassified = []
        # else: leave unclassified as-is (will become single-end)
    
    # =========================================================================
    # Build sample list
    # =========================================================================
    if r1_files:
        # We have R1 files — pair each with its R2
        samples = []
        for r1_path in sorted(r1_files):
            r1_name = r1_path.name
            r2_path = None
            
            # Try substitutions to find R2
            r2_subs = [
                ('_R1_001.', '_R2_001.'),
                ('_R1.', '_R2.'),
                ('.R1.', '.R2.'),
            ...
            for r1_pat, r2_repl in r2_subs:
                if r1_pat in r1_name:
                    r2_name = r1_name.replace(r1_pat, r2_repl, 1)
                    potential_r2 = r1_path.parent / r2_name
                    if potential_r2.exists():
                        r2_path = potential_r2
                        break
            
            # Extract sample ID (remove extension, then R1 indicator)
            sample_id = r1_name
            for suffix in ['.fastq.gz', '.fq.gz', '.fastq', '.fq']:
                sample_id = sample_id.replace(suffix, '')
            # Remove R1 indicator — stop after first match
            for pat in ['_R1_001', '_R1', '.R1', '_1']:
                if pat in sample_id:
                    sample_id = sample_id.replace(pat, '', 1)
                    break
        ...
        read_type = 'paired' if any(s.fastq_2 for s in samples) else 'single'
        return samples, read_type
    
    # No R1 files at all — treat everything as single-end
    samples = []
    for f in sorted(all_files):
        # Skip files already identified as R2
        if str(f) in r2_files_set:
            continue
        sample_id = f.name
        for suffix in ['.fastq.gz', '.fq.gz', '.fastq', '.fq']:
            sample_id = sample_id.replace(suffix, '')
        samples.append(SampleInfo(sample_id=sample_id, fastq_1=f))

...
# Main content
st.title("🤖 Hybrid Pipeline Recommender - Module 4")
st.markdown("### Intelligent pipeline selection using rule-based + ML approaches")
...
with st.expander("ℹ️ How to use this module"):
    ...
    **Purpose:** Get the best DE pipeline recommendation for your data using TWO complementary approaches.
    ...
    ### **1️⃣ Rule-Based Recommender** ✅ Always Available
    ...
    **Advantages:**
    ...
    ### **2️⃣ ML-Based Recommender** 🤖 If Model Available
    ...
    **Advantages:**
    ...
    ### **3️⃣ Comparison Mode** 🔍 Best of Both Worlds
    **When both available:**
    ...
    **Agreement Analysis:**
    ...
    - ⚠️ **Disagree:** Investigate further, consider alternatives
...
if st.session_state.get('m3_complete', False):
    ...
    st.info("ℹ️ No profile from Module 3. Please run Data Profiler first, or upload profile below.")
...
st.markdown("## 2️⃣ Recommender Setup")
...
with col2:
    ...
    if ML_RECOMMENDER_AVAILABLE and ml_model_available:
        ...
        st.warning("⚠️ **Module Available** - No trained model")
...
if st.session_state.get('m4_complete', False):
    ...
    if st.session_state.get('m4_rule_complete', False):
        ...
        # Warnings
        if rec.warnings:
            with st.expander("⚠️ Warnings & Recommendations"):

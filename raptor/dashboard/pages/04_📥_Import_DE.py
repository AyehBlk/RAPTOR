...
# Main content
st.title("📥 Import & Standardize DE Results - Module 7")
st.markdown("### Import differential expression results from multiple pipelines")
...
st.markdown("## 1️⃣ Upload DE Result Files")
...
if uploaded_files:
    ...
    st.markdown("---")
    st.markdown("## 2️⃣ Format Detection & Validation")
    ...
    with col2:
        ...
        if valid_count > 0:
            ...
            if unique_tools >= 2:
                ...
                st.warning("⚠️ Consider adding another method")
...
padj → adjusted_p_value
    ...
    if invalid_files:
        with st.expander("⚠️ Invalid files - click for details"):
    ...
    st.markdown("---")
    st.markdown("## 3️⃣ Significance Thresholds")
    ...
    st.markdown("---")
    st.markdown("## 4️⃣ Data Preview & Quality Check")
    ...
    st.markdown("---")
    st.markdown("## 5️⃣ Import & Standardize All Files")
    ...
    if not valid_files:
        st.warning("⚠️ No valid files to import. Please check file formats above.")
    else:
            ...
            with st.spinner(f"Importing and standardizing {len(valid_files)} file(s)..."):
                ...
                try:
                    ...
                    if de_results_dict:
                        ...
                        if import_errors:
                            with st.expander("⚠️ Some files had errors"):
...
if st.session_state.get('m7_complete', False):
    ...
    with col4:
        avg_sig_pct = np.mean([r.calculate_metrics()['percent_significant'] 
    ...
    for file_name, de_result in de_results.items():
        ...
        summary_data.append({
            ...
            'Total Genes': f"{metrics['total_genes']:,}",
            'Significant': f"{metrics['n_significant']:,}",
            'Up-regulated': f"{metrics['n_up']:,}",
            'Down-regulated': f"{metrics['n_down']:,}",
            '% Significant': f"{metrics['percent_significant']:.1f}%"
        })
    ...
    with col3:
        ...
        if len(de_results) >= 2:
            ...
            st.warning("⚠️ 2+ files recommended")

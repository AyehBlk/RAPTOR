...
st.caption("Comprehensive data profiling and feature extraction for RNA-seq datasets")
...
if st.session_state.get('m2_complete', False):
    ...
    st.info("Using data from Module 1 (Quality check skipped)")
    count_data = st.session_state.get('m1_counts')
    metadata = st.session_state.get('m1_sample_sheet')
    data_source = "module1"
else:
    st.info("Modules 1 & 2 are optional. Please upload count matrix below.")
    ...
    if uploaded_metadata:
        try:
            ...
            if 'sample' not in metadata.columns and 'sample_id' not in metadata.columns:
                ...
                st.warning("No 'condition' or 'group' column found. Group analysis will be limited.")
            else:
    ...
    if st.button("Run Data Profiling", type="primary", use_container_width=True):
        ...
        with st.spinner("Running comprehensive data profiling..."):
            try:
                ...
                st.success("""
                **Data profiling complete.** Extracted 32 statistical features:
...
if st.session_state.get('m3_complete', False) and not st.session_state.get('m3_running', False):
    st.markdown("---")
    st.markdown("## Data Profile Results")
    ...
    .prof-label { font-size: 0.75rem; color: #888; text-transform: uppercase; letter-spacing: 0.4px; }
    .prof-value { font-size: 1.4rem; font-weight: 600; color: #333; margin-top: 2px; }
    .prof-sub { font-size: 0.7rem; color: #aaa; margin-top: 2px; }
    ...
    with col1:
        ...
        bcv_status = "Good" if profile.bcv <= 0.4 else "Moderate" if profile.bcv <= 0.6 else "High"
        st.metric("BCV", f"{profile.bcv:.3f}", delta=bcv_status, delta_color="normal" if profile.bcv <= 0.4 else "inverse")
    ...
    with col2:
        ...
        replicates_ok = "Yes" if profile.has_sufficient_replicates else "No"
        st.metric("Sufficient Replicates", replicates_ok)
    ...
    with st.expander("Detailed Sample Characteristics"):
        ...
        with col2:
            ...
            st.write(f"- Sufficient replicates (≥3): {'Yes' if profile.has_sufficient_replicates else 'No'}")

...
st.markdown("### Combine multiple DE methods for robust consensus")
...
st.markdown("## 1️⃣ Select DE Results")
...
if len(de_results) < 2:
    st.warning(f"⚠️ Only {len(de_results)} result. Ensemble works best with ≥2 methods.")
    ...
    with st.expander("📊 Quick Overlap"):
        ...
        if overlaps:
            ...
            if avg_j >= 0.6:
                ...
                st.warning(f"⚠️ Low overlap (avg: {avg_j:.2f})")

st.markdown("---")

# Section 2: Method Selection
st.markdown("## 2️⃣ Select Ensemble Method")
...
st.markdown("## 4️⃣ Run Ensemble Analysis")
...
if st.button("🚀 Run Ensemble Analysis", type="primary", use_container_width=True):
    ...
    if len(selected_files) < 2:
        st.warning("⚠️ Only 1 method - ensemble works best with 2+")
    ...
    with st.spinner("Running ensemble analysis..."):
        ...
        try:
            ...
            for name, de_result in selected_de_results.items():
                result_obj = SimpleNamespace(data=de_result.results_df.reset_index())
...
if st.session_state.get('m9_complete', False):
    st.markdown("---")
    st.markdown("## 5️⃣ Ensemble Results")

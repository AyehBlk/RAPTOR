#!/usr/bin/env Rscript
# =============================================================================
# RAPTOR v2.2.0 - Module 6: limma-voom Differential Expression Analysis
# =============================================================================
#
# This script performs differential expression analysis using limma-voom.
...
suppressPackageStartupMessages({
    library(optparse)
    library(limma)
    library(edgeR)  # For DGEList and voom normalization
    library(jsonlite)
})
...
option_list <- list(
    ...
    make_option(c("--voom-method"), type = "character", default = "voom",
                help = "Voom method: voom, voomWithQualityWeights [default: voom]"),
    make_option(c("--robust"), action = "store_true", default = FALSE,
                help = "Use robust empirical Bayes [default: FALSE]"),
    make_option(c("--trend"), action = "store_true", default = FALSE,
                help = "Allow mean-variance trend in eBayes [default: FALSE]"),
...
parser <- OptionParser(
    ...
    description = paste(
        "\n🦖 RAPTOR v2.2.0 - limma-voom Differential Expression Analysis",
...
cat("\n")
cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║     🦖 RAPTOR v2.2.0 - limma-voom Analysis (Module 6)       ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n")
cat("\n")
...
cat("\n⚙️ Applying voom transformation (", args$`voom-method`, ")...\n", sep = "")

if (args$`voom-method` == "voomWithQualityWeights") {
    v <- voomWithQualityWeights(y, design, plot = FALSE)
    cat("   Using sample quality weights\n")
} else {
...
cat("\n⚙️ Fitting linear model...\n")

fit <- lmFit(v, design)
fit <- eBayes(fit, robust = args$robust, trend = args$trend)

if (args$robust) cat("   Using robust empirical Bayes\n")
if (args$trend) cat("   Using mean-variance trend\n")
...
res_table <- topTable(fit, coef = ncol(design), number = Inf, 
                     adjust.method = "BH", sort.by = "P")

res_df <- data.frame(
    gene_id = rownames(res_table),
    logFC = res_table$logFC,
    AveExpr = res_table$AveExpr,
    t = res_table$t,
    P.Value = res_table$P.Value,
    adj.P.Val = res_table$adj.P.Val,
    B = res_table$B,
...
res_df$baseMean <- 2^res_df$AveExpr
res_df$stat <- res_df$t

# Add direction and significance
res_df$direction <- ifelse(res_df$logFC > 0, "up",
                          ifelse(res_df$logFC < 0, "down", "unchanged"))

res_df$is_significant <- res_df$adj.P.Val < args$fdr & abs(res_df$logFC) > args$lfc
...
summary_list <- list(
    ...
    parameters = list(
        ...
        voom_method = args$`voom-method`,
        robust_ebayes = args$robust,
        trend = args$trend,
...
if (args$plots) {
    ...
    title(main = "Voom: Mean-Variance Trend")
    
    # MD plot
    plotMD(fit, main = "limma-voom MD Plot")
    abline(h = c(-args$lfc, args$lfc), col = "blue", lty = 2)
    
    # SA plot
    plotSA(fit, main = "Sigma vs Average")
    
    # Volcano plot
    if (requireNamespace("ggplot2", quietly = TRUE)) {
        volcano_df <- res_df
        volcano_df$neg_log10_padj <- -log10(volcano_df$adj.P.Val)
        ...
        p <- ggplot2::ggplot(volcano_df, 
                            ggplot2::aes(logFC, neg_log10_padj, color = significance)) +
...
cat("\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("  ✅ limma-voom ANALYSIS COMPLETE!\n")
cat("═══════════════════════════════════════════════════════════════\n")

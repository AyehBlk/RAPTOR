...
class SampleSheet:
    ...
    @property
    def is_paired(self) -> bool:
        """Check if all (or most) samples are paired-end."""
        if not self.samples:
            return False
        paired_count = sum(1 for s in self.samples if s.is_paired)
        return paired_count > len(self.samples) / 2
    ...
    def get_sample(self, sample_id: str) -> Optional[Sample]:
        ...
        -------
        Sample or None
            The matching sample, or None if not found
        """
    ...
    def validate(self, check_files: bool = False) -> tuple:
        """
        Validate sample sheet structure and optionally file existence.
        
        Parameters
        ----------
        check_files : bool
            If True, check that FASTQ files exist on disk
        ...
        # Check for samples
        if not self.samples:
            errors.append("No samples found in sample sheet")
            return False, errors
        ...
        # Check FASTQ files exist (if requested)
        if check_files:
            import os
            for sample in self.samples:
                if not os.path.exists(sample.fastq_r1):
                    errors.append(f"R1 not found for {sample.sample_id}: {sample.fastq_r1}")
                if sample.is_paired and not os.path.exists(sample.fastq_r2):
                    errors.append(f"R2 not found for {sample.sample_id}: {sample.fastq_r2}")
